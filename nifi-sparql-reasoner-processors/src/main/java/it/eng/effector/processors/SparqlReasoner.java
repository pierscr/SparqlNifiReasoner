...
public class SparqlReasoner extends AbstractProcessor {
        ...
        try{
            flowFile = session.write(flowFile,new StreamCallback() {
                ...
                public void process(InputStream inputStream, OutputStream outputStream) throws IOException {
                    final String rulesFolder=context.getProperty(SPARQL_RULES_FOLDER).getValue();
                    SailRegistry.getInstance().add(new MultipleRulesInferenceFactory());
                    Repository repo = new SailRepository(new MultipleRulesInference(new MemoryStore(), QueryLanguage.SPARQL, new File(rulesFolder)));
                    RepositoryConnection con=repo.getConnection();
                    try {
                        con.add(inputStream, RDFFormat.TURTLE);
                        RDFHandler writer = Rio.createWriter(RDFFormat.TURTLE, outputStream);
